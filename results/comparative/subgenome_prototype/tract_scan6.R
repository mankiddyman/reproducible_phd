...
meta <- read_tsv("wgd6/tip_meta.tsv", show_col_types = FALSE) %>%
  left_join(bed %>% transmute(gene = id, genome, mid = (start + end)/2),
            by = c("gene", "genome"))
key <- function(x) gsub("@", "_", gsub("['\"]", "", x))

fs <- list.files("wgd6/tre", "\\.tre$", full.names = TRUE)
cat(sprintf("trees: %d\n", length(fs)))
...
one <- function(f) {
  ...
  do.call(rbind, lapply(j, function(k) {
    dA <- nd[getMRCA(tr, c(tr$tip.label[k], tA))]
    dB <- nd[getMRCA(tr, c(tr$tip.label[k], tB))]
    ...
    data.frame(pair = p1, chrA = m$chr[iA], chrB = m$chr[iB],
               ...
               sp_gene = m$gene[k],
               vote = ifelse(dA < dB, "A", "B"), stringsAsFactors = FALSE)
  }))
}
...
ggsave("FIG13b_tract_scan6.png", p, width = 12, height = 11, dpi = 170, device = agg_png)
write_csv(gene, "tract_gene_calls6.csv"); write_csv(per_lin, "tract_per_lineage6.csv")
cat("\nWROTE: FIG13b_tract_scan6.png tract_votes6.csv tract_gene_calls6.csv tract_per_lineage6.csv\n")
