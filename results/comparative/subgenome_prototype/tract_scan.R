...
fs <- list.files("wgd/tre", "\\.tre$", full.names = TRUE)
cat(sprintf("trees: %d\n", length(fs)))
...
ggsave("FIG13_tract_scan.png", p, width = 12, height = 11, dpi = 170, device = agg_png)
write_csv(gene, "tract_gene_calls.csv"); write_csv(per_lin, "tract_per_lineage.csv")
cat("\nWROTE: FIG13_tract_scan.png tract_votes.csv tract_gene_calls.csv tract_per_lineage.csv\n")
